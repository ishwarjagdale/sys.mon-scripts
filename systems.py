...
class Systems:
    ...
    @staticmethod
    def load_config():
        if not os.path.exists(os.path.join(os.path.expanduser("~"), 'sysmon_agent.config')):
            return False
        try:
            with open(os.path.join(os.path.expanduser("~"), 'sysmon_agent.config'), 'r') as config:
    ...
    def save_config(configurations):
        try:
            with open(os.path.join(os.path.expanduser("~"), "sysmon_agent.config"), 'w') as config:
